...
<data class="col-md-12">
      ...
      <div class="panel-heading">Форма редактирвоания системного пользователя</div>
          ...
          <fieldset>
            ...
            <div class="form-group">
              <label class="col-md-4 control-label" for="u_password">Пароль</label>  
                <div class="col-md-4">
                  <input id="u_password" name="u_password" value="<?=$u_password?>" type="password" placeholder="" class="form-control input-md" >
                </div>
            </div>
            <div class="form-group">
              <label class="col-md-4 control-label" for="u_password2">Подтверждение пароля</label>  
                <div class="col-md-4">
                  <input id="u_password2" name="u_password2" value="<?=$u_password2?>" type="password" placeholder="" class="form-control input-md" >
                </div>
            </div>                                  
            <div class="form-group">
            ...
            <div class="form-group">
              <label class="col-md-4 control-label" for="u_type">Тип пользователя</label>  
                <div class="col-md-4">
                  ...
                  <option value="1" <?=$u_type == 1 ? 'selected' : ''?>>Системный пользователь</option>
                  <option value="2" <?=$u_type == 2 ? 'selected' : ''?>>Родитель</option>
                  </select>
                </div>
            </div>            
            <?=$this->userRightsTable($u_id, $u_type)?>
            <div class="form-group" id="students_data" style="<?=$u_type == 2 ? '' : 'display:none;'?>">
              <label class="col-md-4 control-label" for="st_id">Родитель следующих детей:</label>  
                <div class="col-md-4">
                    <?php while($student = $students->fetch_assoc()){?>
                        <div class="checkbox">
                        <label><input type="checkbox" name="students[]" value="<?=$student['st_id']?>" <?=$this->checkParentStudent($u_id, $student['st_id'])?>><?=$student['st_second_name'].' '.$student['st_first_name'].' '.$student['st_third_name']?></label>
                        </div>
                    <?php }?>                  
                </div>
            </div>            
            <div class="form-group">
              <label class="col-md-4 control-label" for="u_active">Активен</label>  
                <div class="col-md-1">
                    <div class="checkbox">
                        <input type="checkbox" id="u_active" name="u_active" value="1" <?=$u_active ? 'checked' : ''?>>
                    </div>
                </div>
            </div>
            <hr>
            <div class="form-group">
              ...
              <div class="col-md-4">
                <button type="submit" id="btn" class="btn btn-primary btn-block">Редактировать пользователя</button>
              </div>  
            </div>
          </fieldset>
        </form>
